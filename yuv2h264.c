...
#define INPUT_ENCODING MMAL_ENCODING_I420
#define OUTPUT_ENCODING MMAL_ENCODING_H264
#define ZERO_COPY MMAL_TRUE

#define INPUT_POOL_SIZE 3
...
o_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
    ...
    status = mmal_port_send_buffer(oport, buffer);
...
/* configure the encoder input port */
static void
setup_input_port(void)
{
  	MMAL_STATUS_T status;
    iport = encoder->input[0];
    ...
    iport->buffer_num = INPUT_POOL_SIZE;

    status = mmal_port_format_commit(iport);
    if (status != MMAL_SUCCESS) {
        printf("mmal_port_format_commit(iport) failed\n");
        exit(1);
    }
    ...
    ipool = mmal_port_pool_create(iport, INPUT_POOL_SIZE, iport->buffer_size);
    if (ipool == NULL) {
...
setup_output_port(void)
{
    ...
    oport = encoder->output[0];

    /* Rather than setting up from scratch copy the input port format
     * across.
     */
    //mmal_format_copy(oport->format, iport->format);
    debug_dump_port(oport);
    oport->format->encoding = OUTPUT_ENCODING;
    oport->format->bitrate = 10000000;
    oport->format->es->video.frame_rate.num = 0;
    oport->format->es->video.frame_rate.den = 1;
    oport->buffer_size = oport->buffer_size_recommended;

    if (oport->buffer_size < oport->buffer_size_min)
        oport->buffer_size = oport->buffer_size_min;

    oport->buffer_num = oport->buffer_num_recommended;

    if (oport->buffer_num < oport->buffer_num_min)
        oport->buffer_num = oport->buffer_num_min;
...
main(int argc, char **argv)
{
    ...
    if (argc < 3) {
        printf("usage: %s output inputs...\n", argv[0]);
        exit(1);
    }

    output_file = argv[1];

    /* Create the Video Encoder MMAL component. We make things simple and store in a global */
    status = mmal_component_create("vc.ril.video_encode", &encoder);
    if (status != MMAL_SUCCESS) {
        printf("Failed to create vc.ril.video_encode component\n");
        exit(1);
    }
    ...
    output_fd = open(output_file, O_CREAT | O_WRONLY, S_IRUSR | S_IWUSR);
    if (output_fd < 0) {
